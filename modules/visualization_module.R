    ...
    observe({
      ...
      lapply(categorical_vars, function(var) {
        output[[paste0("plot_", var)]] <- renderPlot({
          ...
          plot_data[[var]] <- factor(plot_data[[var]], levels = levels)  # Ensure the variable is a factor with the correct levels
          
          ggplot(plot_data, aes(x = .data[[var]], fill = .data[[var]])) + 
            geom_bar() + 
            theme_classic() +  # Changed to theme_classic
            labs(title = variable_labels[[var]], x = NULL, y = "Count") +  # Remove x axis label
            ...
            theme(
              ...
              plot.title = element_text(size = 14, face = "bold"),  # Make titles bold and larger
              axis.title = element_text(size = 12, face = "bold"),  # Make axis titles bold and larger
              legend.title = element_text(size = 12, face = "bold"),  # Make legend titles bold and larger
              legend.text = element_text(size = 10),  # Adjust legend text size
              plot.margin = margin(10, 10, 10, 10)  # Add margin to the plot
            ) +
            guides(fill = guide_legend(nrow = 2, byrow = TRUE)) +
            geom_text(stat = 'count', aes(label = ..count..), vjust = -0.5, size = 3)  # Add data labels on bars and adjust size
        })
      })
      ...
      lapply(continuous_vars, function(var) {
        output[[paste0("plot_", var)]] <- renderPlot({
          ...
          ggplot(plot_data, aes(x = .data[[var]])) + 
            geom_density(adjust = 1.5, fill = "blue", alpha = 0.3) +  # Adjust fill transparency
            theme_classic() +  # Changed to theme_classic
...
# Example call to run the app
# shinyApp(ui = visualizationUI("visualization"), server = visualizationServer)